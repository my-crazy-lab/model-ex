"""
Training configuration for Adapter Tuning implementation
"""
...
class TrainingConfig:
    ...
    def __post_init__(self):
        ...
        # Set adapter learning rate if not specified
        if self.adapter_learning_rate is None:
            self.adapter_learning_rate = self.learning_rate * self.adapter_lr_multiplier
    ...
    def get_optimizer_params(self, model) -> List[Dict[str, Any]]:
        """Get optimizer parameters with different learning rates for adapters"""
        if not self.train_adapters_only:
            return [{"params": model.parameters(), "lr": self.learning_rate}]
        
        # Separate adapter and base model parameters
        adapter_params = []
        base_params = []
        ...
        if base_params and not self.freeze_base_model:
            optimizer_params.append({
                "params": base_params,
...
MULTI_TASK_TRAINING = TrainingConfig(
    num_train_epochs=5,
    per_device_train_batch_size=16,
    learning_rate=1e-3,
    eval_steps=500,
